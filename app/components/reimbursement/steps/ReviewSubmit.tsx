  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white p-6 rounded-lg shadow-sm border border-charcoal/10">
        ...
        {/* Requester Info */}
        <div className="mb-6">
          <h3 className="text-sm font-medium text-charcoal/50 uppercase tracking-wide mb-3">
            Check Request Information
          </h3>
          <div className="bg-warm-white p-4 rounded-lg space-y-2 text-charcoal">
            <p><span className="font-medium text-charcoal/80">Payable to:</span> <span className="text-charcoal">{data.requester.payableTo}</span></p>
            <p><span className="font-medium text-charcoal/80">Email:</span> <span className="text-charcoal">{data.requester.email}</span></p>
            {data.requester.phone && (
              <p><span className="font-medium text-charcoal/80">Phone:</span> <span className="text-charcoal">{data.requester.phone}</span></p>
            )}
            <p><span className="font-medium text-charcoal/80">Address:</span> <span className="text-charcoal">{data.requester.address}</span></p>
            <p><span className="font-medium text-charcoal/80">Date of Request:</span> <span className="text-charcoal">{formatDate(data.requester.dateOfRequest)}</span></p>
            <p><span className="font-medium text-charcoal/80">Date Check Needed:</span> <span className="text-charcoal">{formatDate(data.requester.dateCheckNeeded)}</span></p>
            {data.requester.invoiceNumber && (
              <p><span className="font-medium text-charcoal/80">Invoice #:</span> <span className="text-charcoal">{data.requester.invoiceNumber}</span></p>
            )}
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="mt-4 flex justify-end">
            <div className="text-right">
              <p className="text-sm text-charcoal/50">Total Amount</p>
