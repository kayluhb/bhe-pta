...
export async function action({ request, context }: Route.ActionArgs) {
	try {
		const denied = await requireTurnstile(
			request,
		...
		if (!geminiResponse.ok) {
			...
			return Response.json(
				{
					error:
						"Failed to process image with Gemini. Please try uploading it directly.",
				},
				{ status: 502 },
			);
		}
		...
		if (!extractedText) {
			return Response.json(
				{
					error:
						"Could not extract text from image. Please upload the file directly.",
				},
				{ status: 422 },
			);
		}
		...
		doc.text(
			"Automatically transcribed from uploaded image.",
			105,
			pageHeight - 10,
			{ align: "center" },
		);
