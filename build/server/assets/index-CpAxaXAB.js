import { C as CredentialsProviderError, e as chain, d as getProfileName, r as readFile, h as homedir, H as HttpRequest } from "./SignatureV4MultiRegion-B05s6TWe.js";
import { s as setCredentialFeature } from "./index-DgMOBvXs.js";
import { createHash, createPrivateKey, createPublicKey, sign } from "node:crypto";
import { f as fs } from "./server-build-Bwh1Gzls.js";
import { dirname, join } from "node:path";
import { p as parseKnownFiles } from "./index-BTEtc6RR.js";
import "path";
import "buffer";
import "stream";
import "crypto";
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      ...
      const { fromContainerMetadata } = await import("./index-BcdqI39r.js");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("./index-BcdqI39r.js");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("./index-BTEtc6RR.js").then((n) => n.i);
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData2) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("./index-DMC-7VEM.js");
...
class LoginCredentialsFetcher {
  ...
  async refresh(token) {
    const { SigninClient, CreateOAuth2TokenCommand } = await import("./index-Do6lC3ju.js");
...
const resolveProcessCredentials = async (options, profile) => import("./index-C_s6u1d2.js").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
...
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("./index-BcdX9OhH.js").then(({ fromTokenFile }) => fromTokenFile({
