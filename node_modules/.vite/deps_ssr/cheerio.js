...
var hasOwn = (
  ...
  (_a2 = Object.hasOwn) !== null && _a2 !== void 0 ? _a2 : ((object, prop2) => Object.prototype.hasOwnProperty.call(object, prop2))
);
var rspace = /\s+/;
...
function findFilterElements(root2, selector, options, queryForSelector, totalLimit) {
  ...
  if (limit === 0)
    return [];
  const elemsNoLimit = sub.length === 0 && !Array.isArray(root2) ? getChildren(root2).filter(isTag) : sub.length === 0 ? (Array.isArray(root2) ? root2 : [root2]).filter(isTag) : queryForSelector || sub.some(isTraversal) ? findElements(root2, [sub], options, limit) : filterElements(root2, [sub], options);
...
function filterArray(nodes, match, xmlMode, root2) {
  return typeof match === "string" ? filter(match, nodes, { xmlMode, root: root2 }) : nodes.filter(getFilterFn(match));
}
...
function getLoad(parse6, render2) {
...
var htmlDecoder2 = getDecoder2(htmlDecodeTree2);
var xmlDecoder2 = getDecoder2(xmlDecodeTree2);
