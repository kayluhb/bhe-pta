...
const SKIP_EXTENSIONS = new Set([
  ".php", ".txt", ".css", ".js", ".htaccess", ".lock", ".ds_store", ".log",
  ".html", ".htm", ".xml", ".json", ".sql", ".map", ".woff", ".woff2",
  ".ttf", ".eot", ".svg", ".ico",
]);
...
const SKIP_DIRS = new Set([
  "pb_backupbuddy", "backupbuddy_backups", "wpsc", "eshop_files",
]);

const STOCK_KEYWORDS = [
  "accomplishment", "achievement", "shutterstock", "stock",
  "depositphotos", "istock", "getty",
];
  ...
  try {
    ...
    const candidates = files
      ...
      .map((f) => {
        ...
        return { file: f, w: parseInt(match[1]), h: parseInt(match[2]) };
      })
...
function main() {
  ...
  for (const year of sortedYears) {
    ...
    totalSize += yearSize;
    console.log(
      `    ${year}: ${entries.length} files (${(yearSize / 1024 / 1024).toFixed(1)} MB)`
    );
  }
  ...
  for (const entry of allEntries) {
    ...
    if (isImage) {
      ...
      if (isCakeGallery) {
        const cakeThumb = join(
          GALLERY_DIR,
          "cakes",
          "thumbs",
          `thumbs_${entry.filename}`
        );
