...
if (!homeDir) {
  throw new Error('HOME must be set to locate the WordPress SQL dump.');
}
...
console.log(
  `Found ${terms.length} terms, ${termTaxonomies.length} taxonomies, ${termRelationships.length} relationships`,
);

// ─── Filter & Process Posts ───────────────────────────────────────────────────

const publishedPosts = posts.filter((p) => p.postType === 'post' && p.postStatus === 'publish');
...
function stripShortcodes(html: string): string {
  ...
  let result = html.replace(/\[caption[^\]]*\]([\s\S]*?)\[\/caption\]/gi, '$1');
...
for (const p of publishedPosts) {
  ...
  const excerpt = p.excerpt.trim() ? htmlToExcerpt(p.excerpt) : htmlToExcerpt(content);
