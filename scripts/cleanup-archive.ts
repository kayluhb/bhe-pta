...
const JUNK_TITLES_EXACT = new Set([
  "cc", "them", "unnamed", "image", "photo", "logo", "LOGO",
  "email", "website", "Cleardot", "fake-background",
  "facebook", "Facebook Circle", "Download on Google Play",
  "Download on the App Store", "Amazon Smile 2", "Amazon Smile",
  "mask", "sandwich", "Socks", "Poker", "Peppers",
]);
...
const CLIP_ART_KEYWORDS = [
  "pineapple with sunglasses", "heart hands", "paint brushes", "gift box",
  "love hands", "alarm clock", "popsicles", "coffee and kleenex",
  "raking leaves", "hawaiian shirt", "red ridinghood", "medieval dragon",
  "bouncy house clip", "clip art", "clipart",
];

// UI/navigation artifacts
const UI_KEYWORDS = [
  "app store", "google play", "amazon smile", "cleardot", "fake-background",
  "facebook circle", "fb-icon", "fb_icon", "instagram-icon", "twitter-icon",
];
...
function isSponsorLogo(item: ManifestItem): boolean {
  ...
  const businessLogoPatterns = [
    ...
    "waterloo", "brightwatch", "pinthouse", "elm restaurant",
    "endeavor acadmics", "stinson moyle", "outside voice",
    "traveling photo booth", "whole earth", "rk group",
    "smile 360", "south austin therapy", "spyglass",
    "wells", "west holistic", "westlake anesthesia",
    "realty austin", "davis agency", "design trait",
    "easy tiger logo", "mpa", "aci consulting",
    "pinnery", "loot rental", "loot finer",
    "barton hills mortgage", "elm-restaurant",
    "peoplesrx", "supersmiles", "tourwick",
    "the abgb", "the davis agency", "austin city dental",
    "artgarage", "kw logo", "wf logo",
  ];
...
function deleteFromR2(r2Key: string): boolean {
  try {
    execSync(
      `wrangler r2 object delete "pta-archive/${r2Key}" --remote`,
...
function main() {
  ...
  for (const item of items) {
    if (BAD_TITLE_FIX.has(item.title)) {
      ...
      item.title = newTitle;
      item.id = fb.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      titlesFixed++;
    }
  }
