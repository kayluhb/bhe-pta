...
function main() {
  ...
  const attachments: WpAttachment[] = JSON.parse(readFileSync(ATTACHMENTS_FILE, 'utf-8'));
  ...
  for (const item of existingItems) {
    ...
    if (wpAtt) {
      ...
      if (wpAtt.title && wpAtt.title.length > 1) {
        ...
        const filenameTitle = titleCase(basename(filename, extname(filename)));
  ...
  console.log(`  Items after removing 2022-2023: ${processedItems.length}`);
